...
import { terminateUserSessions } from "@/lib/Api/auth/terminateUserSessions"
...
console.log(assignedUsers, "assignedUsers");
  ...
  const handleTerminateUser = async (user: User) => {
    try {
      await terminateUserSessions({ userId: user.id })
      toast({
        title: "Success",
        description: `${user.firstName} ${user.lastName} has been logged out from all devices`,
      })
    } catch (error: any) {
      toast({
        title: "Error",
                          ...
                          <DropdownMenuContent align="end" className="dark:bg-gray-800 dark:border-gray-700">
                            ...
                            {currentUser?.role === "POSPORT_ADMIN" && (
                              <DropdownMenuItem 
                                onClick={(e) => {
                                  e.stopPropagation()
                                  handleTerminateUser(user)
                                }} 
                                className="text-orange-600 dark:text-orange-400 dark:hover:bg-orange-900/20"
                              >
